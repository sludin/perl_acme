...
my $challenges = { 'www.example.com' =>   [ "host1", "~/www/.well-known/acme-challenge" ],
                   'cloud.example.com' => [ "host2", "/opt/local/www/htdocs/.well-known/acme-challenge" ] };

eval
{

  my $acme = ACME->new( host               => $host,
...
if ( $@ )
{
  die $@ if ref $@ ne "LEACME::Exception";
